...
;(async () => {
  ...
  await sleep(250)
  const markerPath = path.join(OUT, 'debug-marker.png')
  await page.screenshot({ path: markerPath, clip: { x: 0, y: 0, width: 1200, height: 800, scale: 1 } })
  const img = decodePNG(fs.readFileSync(markerPath))
  const px = []
