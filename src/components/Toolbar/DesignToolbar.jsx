  ...
  return <>
    ...
    {accountPanel && <aside className="side-panel" ref={accountPanelRef} role="dialog" aria-label={ACCOUNT_PANELS[accountPanel]}>
      <header className="side-panel-head">
        ...
        <button type="button" title="Close" aria-label="Close panel" onClick={closeAccountPanel}><Icon name="x" /></button>
      </header>
      <div className="side-panel-body">
        {accountPanel === 'profile' && <>
          <div className="side-panel-profile-card">
            <span className="side-panel-avatar">{userName.trim().charAt(0).toUpperCase() || '?'}</span>
        ...
        {accountPanel === 'appearance' && <>
          ...
          <div className="side-theme-row">
            <button type="button" role="radio" aria-checked={!darkMode} className={!darkMode ? 'is-selected' : ''} onClick={() => setDarkMode(false)}><Icon name="sun" /><span>Light</span></button>
            <button type="button" role="radio" aria-checked={darkMode} className={darkMode ? 'is-selected' : ''} onClick={() => setDarkMode(true)}><Icon name="moon" /><span>Dark</span></button>
          </div>
          <p className="side-hint">The board canvas, toolbars and panels all follow this theme instantly.</p>
        </>}
        {accountPanel === 'help' && <>
