...
const PROFILE_KEY = 'kanvasProfile'
...
function Icon({ name }) {
  ...
  const thatColor = name === 'eraser' ? (darkMode ? '#ffffff' : '#000000') : 'currentColor'
  const icons = {
    ...
    text: <><path d="M4 6V4h16v2M12 4v16M8 20h8" {...common} /></>, eraser: <path d="m7 19-3-3a2 2 0 0 1 0-3l7-7a2 2 0 0 1 3 0l5 5a2 2 0 0 1 0 3l-5 5H7Z" {...common} stroke={thatColor} />, image: <><rect x="3" y="4" width="18" height="16" rx="2" {...common} /><circle cx="8" cy="9" r="1" {...common} /><path d="m21 15-5-5L5 20" {...common} /></>,
...
function Toolbar({ stageRef, onImageUpload, imageInputRef, view, onZoomReset, lastSavedAt, onNewBoard, onSaveBoard, onSaveAsBoard, onDeleteBoard, beginProcessing, finishProcessing, showUserMessage }) {
  ...
  const [userName, setUserName] = useState(() => {
    try {
      ...
      if (raw) { const p = JSON.parse(raw); if (p && p.displayName) return p.displayName }
  ...
  const [userEmail, setUserEmail] = useState(() => {
    try {
      const raw = localStorage.getItem(PROFILE_KEY)
      if (raw) { const p = JSON.parse(raw); if (p && p.email) return p.email }
    } catch { /* corrupted — fall back */ }
  ...
  const backToAccountMenu = () => { setAccountPanel(null); setProfileError('') }
  const saveProfile = () => {
    ...
    setProfileError('')
    try {
      localStorage.setItem(PROFILE_KEY, JSON.stringify({ displayName: name, email }))
    } catch { /* storage full or blocked — silently fail */ }
    setUserName(name)
    setUserEmail(email)
    setProfileSaved(true)
    showUserMessage?.('Profile saved successfully.')
  ...
  return <>
    ...
    {accountPanel && <aside className="side-panel" ref={accountPanelRef} role="dialog" aria-label={ACCOUNT_PANELS[accountPanel]}>
      <header className="side-panel-head">
        <button type="button" className="side-panel-back" title="Back to board" aria-label="Back to board" onClick={backToAccountMenu}><Icon name="chevron-left" /></button>
      ...
      <div className="side-panel-body">
        {accountPanel === 'profile' && <>
          ...
          <label className="side-field"><span>Display name</span><input ref={profileNameRef} defaultValue={userName} maxLength={48} placeholder="Your name" onChange={() => setProfileError('')} /></label>
          <label className="side-field"><span>Email</span><input ref={profileEmailRef} defaultValue={userEmail} type="email" placeholder="you@example.com" onChange={() => setProfileError('')} /></label>
          {profileError && <span className="side-avatar-error" role="alert">{profileError}</span>}
