...
export default function CanvasStage({ stageRef, view, setView, onCursorMove, onImageDrop, onUserMessage }) {
  ...
  const renderedShapes = useMemo(() => erasedLive.size ? shapes.filter(shape => !erasedLive.has(shape.id)) : shapes, [shapes, erasedLive])
  ...
  // PAN: coalesce high-frequency pointer samples to one React viewport update
  // per animation frame. The last sample still wins, so normal pan behaviour
  // remains unchanged while high-rate trackpads/styli cannot trigger dozens of
  // full canvas/culling passes in one frame.
  const applyPanMove = useCallback(e => {
  ...
  const handlePanMove = useCallback(e => {
    pendingPanMoveRef.current = e
    if (panMoveRafRef.current) return
    panMoveRafRef.current = requestAnimationFrame(() => {
      panMoveRafRef.current = 0
      const pending = pendingPanMoveRef.current
      pendingPanMoveRef.current = null
      if (pending) applyPanMove(pending)
    })
  }, [applyPanMove])
