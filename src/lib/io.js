...
export function exportPNG(stage, fileName = 'diagram') { if (!stage) return; const overlay = stage.findOne('.overlay'); if (overlay) { overlay.hide(); stage.draw() } const url = stage.toDataURL({ pixelRatio: 2 }); if (overlay) { overlay.show(); stage.draw() } fetch(url).then(r => r.blob()).then(blob => download(blob, sanitize(fileName) + '.png')) }
