...
;(async () => {
  ...
  await page.screenshot({ path: 'viewport-tests/debug-sync.png', clip: { x: 0, y: 0, width: 1200, height: 800, scale: 1 } })
  const img = decodePNG(fs.readFileSync('viewport-tests/debug-sync.png'))
  let nOld = 0, nNew = 0
